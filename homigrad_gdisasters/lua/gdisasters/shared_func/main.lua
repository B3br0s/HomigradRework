...
function isOutdoor(ply, isprop)
	...
	if isprop == nil then
		if ply.gDisasters == nil then ply.gDisasters = {} end 
		if ply.gDisasters.Area == nil then ply.gDisasters.Area = {} end 
...
function IsSomethingBlockingWind(entity)
	...
	local tr = util.TraceLine( {
		...
		endpos = entity:GetPos() + Vector(0,0,10) + (GLOBAL_SYSTEM.Current.Atmosphere.Wind.Direction * -300),
		filter = entity
