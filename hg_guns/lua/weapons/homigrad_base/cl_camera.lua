...
function SWEP:Camera(eyePos, eyeAng, view, vecVel, angVel)
	...
	local att = self:GetMuzzleAtt(gun, self.availableAttachments.sight and self.availableAttachments.sight["mount"] and not (not self.attachments or table.IsEmpty(self.attachments.sight) or self.attachments.sight[1] == "empty"))
	wepPos, wepAng = att.Pos, att.Ang
	wepAng:RotateAroundAxis(wepAng:Forward(), -90)
	...
	eyeAngL = eyeAng
	local up, right, forward, forward2 = wepAng:Up(), wepAng:Right(), wepAng:Forward(), ply:EyeAngles():Forward()
	...
	local zoomPos = up * addPos[1] + right * addPos[2] + forward * addPos[3]
	addPos = self.ZoomPosEnd
	local posZoom = originWep + zoomPos - recoilZoomPos / 2
	local _
	--_,angZoom = WorldToLocal(vecZero,wepAng,vecZero,-self.attAng)
	--angZoom[3] = eyeAngL[3]
	angZoom = eyeAngL
	addPos = self.IdlePos
	...
	if zoom then
		...
		if larm == 1 or rarm == 1 then
			...
			local angRand = AngleRand(-k, k)
			angRand[3] = 0
			ply:SetEyeAngles(ply:EyeAngles() + angRand)
		end
	else
		NeedAccarucyRender = nil
		k = LerpFT(0.2 * self.Ergonomics ^ 2, k, 0)
		k2 = LerpFT(0.25, k2, 0)
	end
...
hook.Add("Think", "DOFThink", function()
	if not GAMEMODE:PostProcessPermitted("dof") or not pp_dof:GetBool() or potatopc:GetInt() >= 1 then return end
