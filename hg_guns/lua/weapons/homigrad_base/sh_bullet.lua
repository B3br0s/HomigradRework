...
function SWEP:BulletCallBack(tr,dmginfo)
    ...
    if tr.Entity:Health() or tr.Entity:IsRagdoll() then
        net.Start("hg_reg")
        net.WriteTable(tr)
...
function SWEP:Shoot(isfake)
    ...
    if CLIENT and self:GetOwner() == LocalPlayer() then
        self:PrimarySpread()
        Recoil = Recoil + 2
        RecoilS = Recoil + self.Primary.Force / (self.Primary.Force / 1.25) 
    elseif SERVER then
        self:PrimarySpread()
    end
    ...
    if !isfake then
        self:EmitSound((!istable(self.Primary.Sound) and self.Primary.Sound or table.Random(self.Primary.Sound)),100,math.random(95,110),1,CHAN_WEAPON)
...
function SWEP:ShootNets(isfake)
    if self.NextShoot and self.NextShoot > CurTime() then return end
    if self.reload then
        return
    end
