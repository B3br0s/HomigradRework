	...
	for i,ply in RandomPairs(tbl) do
		...
		if roundActiveName == "tdm" then
			...
			timer.Simple(18.5,function()
				...
				local teamTbl = tdm[tdm.teamEncoder[ply:Team()]]
				local color = teamTbl[2] ply:SetModel(teamTbl.models[math.random(#teamTbl.models)])		
			end)
		end
...
function tdm.PlayerSpawn(ply,teamID)
	...
	if math.random(1,3) == 3 then if ply:Team() == 1 then ply:Give("weapon_f1") else ply:Give("weapon_rgd5") end end
end
