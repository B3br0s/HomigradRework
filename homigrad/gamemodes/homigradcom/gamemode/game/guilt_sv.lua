function GuiltLogic(ply,att,dmgInfo,dontApply)
	if roundActiveName == "SandBox" or roundActiveName == "zombieinfection" or roundActiveName == "homicide" then return end
...
function GuiltCheck(att,ply)
	if ply then return end --hueta
	if att.Guilt >= 100 then
		att.Guilt = 0
		
		if not att.noguilt and not att:HasGodMode() then
			if roundActiveName == "SandBox" or roundActiveName == "zombieinfection" or roundActiveName == "homicide" or roundActiveName == "gangrazborki" then return end
			att:DropWeapon1()
