...
util.AddNetworkString("dropweapona")
util.AddNetworkString("useweaponinb")
util.AddNetworkString("RandomPhrase")
util.AddNetworkString("TraitorPhrase")
...
net.Receive("PurchaseTDM",function(len,ply)
	...
	ply:GiveAmmo(weapons.Get(ItemPurchased).Primary.DefaultClip * 2,weapons.Get(ItemPurchased).Primary.Ammo)

	timer.Simple(0.2,function()
		ply:SelectWeapon(ItemPurchased)
	end)
end)
...
hook.Add("Player Think","Player Respawn Huy",function (ply)
	if ply.CanSpawn == true and not ply:Alive() and roundActiveName == "zombieinfection" then
