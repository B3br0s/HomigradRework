...
SWEP.PrintName 				= "Улучшенное Взрывчатое Устройство"
SWEP.Author 				= "V-City"
SWEP.Instructions			= "ЛКМ - Заложить; ПКМ - Взорвать."
...
if SERVER then
    ...
    local function Bomb(ent)
        ...
        ent:EmitSound("nokia_call1.wav")

        local timea = math.Rand(0.6,0.8)

		timer.Simple(timea,function()
            local SelfPos,PowerMult,Model = ent:LocalToWorld(ent:OBBCenter()),6,ent:GetModel()
            ent:StopSound("nokia_call1.wav")
            ent:EmitSound("snd_jack_fragsplodefar.ogg",125, math.random(90, 120), 1, CHAN_AUTO, 0, 0)
            ParticleEffect("pcf_jack_groundsplode_large",SelfPos,vector_up:Angle())
            util.ScreenShake(SelfPos,99999,99999,1,3000)
            
            sound.Play("snd_jack_impulse.ogg",SelfPos,90,math.random(80,110))

            sound.Play("weapons/mortar/mortar_explode3.wav",SelfPos,100,math.random(80,110))

            sound.Play("snd_jack_plasmapop.ogg",SelfPos,70,math.random(80,110))
            
            sound.Play("snd_jack_fireworksplodeclose.ogg",SelfPos,100,math.random(80,110))

            if util.GetSurfaceIndex(ent:GetBoneSurfaceProp(0)) == 3 or util.GetSurfaceIndex(ent:GetBoneSurfaceProp(0)) == 66 then
                JMod.FragSplosion(ent, SelfPos + Vector(0, 0, 20), 1024, 300, 3500, ent.owner or game.GetWorld())
                JMod.Sploom(ent,SelfPos,150)
			    JMod.Sploom(ent,SelfPos,150)
            end
