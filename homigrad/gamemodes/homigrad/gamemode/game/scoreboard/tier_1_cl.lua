...
local function ToggleScoreboard(toggle)
	if toggle then
		...
		HomigradScoreboard:SetSize(scrw*.5,scrh*.6)
...
HomigradScoreboard.Paint = function(self,w,h)
    ...
    draw.SimpleText("Тикрейт: " .. tick, "HomigradFont", w - 15, h - 25, tick <= 35 and red or green, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
    ...
    for i, ply in pairs(sort) do
		...
		function HomigradScoreboard:AddPlayer(ply)
			...
			playerPanel:SetColor(Color(15, 15, 15, 250))
			playerPanel.DoClick = function()
				...
				playerMenu:AddOption("Respawn", function()
					LocalPlayer():ConCommand("ulx respawn $" .. ply:UserID())
				end)
