...
function jailbreak.HUDPaint_RoundLeft(white2,time)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        draw.DrawText( "ДжейлБрейк", "HomigradFontBig", ScrW() / 2, ScrH() / 8, Color( 255,170,0,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        ...
        if lply:Team() == 2 then
            draw.DrawText( "Ваша задача не дать заключенным сбежать. ВАМ ЗАПРЕЩЕНО УБИВАТЬ ЗАКЛЮЧЕННЫХ БЕЗ ПРИЧИНЫ", "HomigradFontBig", ScrW() / 2, ScrH() / 1.2, Color( 55,55,255,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        else
            draw.DrawText( "Ваша задача выбраться из камеры,найти оружие и убить ФСИН", "HomigradFontBig", ScrW() / 2, ScrH() / 1.2, Color( 255,170,0,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
		end
        return
    end
