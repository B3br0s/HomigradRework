events.GetTeamName = tdm.GetTeamName

local colorSpec = ScoreboardSpec
function events.Scoreboard_Status(ply)
...
function events.HUDPaint_RoundLeft(white2,time)
	local time = math.Round(roundTimeStart + roundTime - CurTime())
	local acurcetime = string.FormattedTime(time,"%02i:%02i")
	local lply = LocalPlayer()
	local name,color = events.GetTeamName(lply)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        draw.DrawText( "Ивент...", "HomigradFontBig", ScrW() / 2, ScrH() / 8, Color( 155,55,55,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER )
	...
	if time > 0 then
		draw.SimpleText("До конца режима events : ","HomigradFont",ScrW() / 2 - 200,ScrH()-25,white,TEXT_ALIGN_LEFT,TEXT_ALIGN_CENTER)
	...
	*/
	...
	if lply:Team() == 3 or lply:Team() == 2 or not lply:Alive() and events.police then
...
function events.PlayerClientSpawn()
