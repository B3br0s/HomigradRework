function events.StartRoundSV(data)
    tdm.RemoveItems()
    ...
    for i,ply in pairs(players) do
		...
		if ply.eventsForceT then
			ply.eventsForceT = nil
...
function events.EndRound(winner) tdm.EndRoundMessage(winner) end

function events.PlayerSpawn(ply,teamID)
	local teamTbl = events[events.teamEncoder[teamID]]
...
function events.PlayerInitialSpawn(ply) ply:SetTeam(2) end

function events.PlayerDeath(ply,inf,att) return false end

function events.NoSelectRandom()
