...
function hideandseek.HUDPaint_RoundLeft(white2,time)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        if lply:Team() == 1 then
            draw.DrawText( "Ваша задача найти и убить всех до прибытия Спецназа", "HomigradFontBig", ScrW() / 2, ScrH() / 1.2, Color( 155,55,55,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        else
            draw.DrawText( "Вас ищут, вам нужно выжить и сбежать по приезду Спецназа", "HomigradFontBig", ScrW() / 2, ScrH() / 1.2, Color( 55,155,55,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
		end
        return
    end
