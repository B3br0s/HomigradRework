furryinfection.GetTeamName = tdm.GetTeamName

local colorSpec = ScoreboardSpec
function furryinfection.Scoreboard_Status(ply)
...
function furryinfection.HUDPaint_RoundLeft(white2,time)
	local time = math.Round(roundTimeStart + roundTime - CurTime())
	local acurcetime = string.FormattedTime(time,"%02i:%02i")
	local lply = LocalPlayer()
	local name,color = furryinfection.GetTeamName(lply)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        draw.DrawText( "Фурри Инфекция", "HomigradFontBig", ScrW() / 2, ScrH() / 8, Color( 155,55,55,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
	...
	*/
	...
	if lply:Team() == 3 or lply:Team() == 2 or not lply:Alive() and furryinfection.police then
...
function furryinfection.PlayerClientSpawn()
