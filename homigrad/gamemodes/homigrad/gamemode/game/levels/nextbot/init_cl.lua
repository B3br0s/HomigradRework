nextbots.GetTeamName = tdm.GetTeamName

local colorSpec = ScoreboardSpec
function nextbots.Scoreboard_Status(ply)
...
function nextbots.HUDPaint_RoundLeft(white2,time)
	local time = math.Round(roundTimeStart + roundTime - CurTime())
	local acurcetime = string.FormattedTime(time,"%02i:%02i")
	local lply = LocalPlayer()
	local name,color = nextbots.GetTeamName(lply)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        draw.DrawText( "НекстБоты", "HomigradFontBig", ScrW() / 2, ScrH() / 8, Color( 255,255,255,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        --draw.DrawText( roundTypes[roundType], "HomigradFontBig", ScrW() / 2, ScrH() / 5, Color( 55,55,155,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
            ...
            draw.DrawText( "Подсказка,не жми на шифт.", "HomigradFontBig", ScrW() / 2, ScrH() / 1.2, Color( 55,155,55,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        return
    end

	if time > 0 then
		draw.SimpleText("До окончания раунда : ","HomigradFont",ScrW() / 2 - 200,ScrH()-25,white,TEXT_ALIGN_LEFT,TEXT_ALIGN_CENTER)
	...
	*/
	...
	if lply:Team() == 3 or lply:Team() == 2 or not lply:Alive() and nextbots.police then
...
function nextbots.PlayerClientSpawn()
