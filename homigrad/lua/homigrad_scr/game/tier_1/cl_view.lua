...
net.Receive("pophead",function(len)
	...
	if !playing and GetConVar("hg_deathsound"):GetBool() then
		...
		timer.Create("DeathCam",0,1,function()
			playing = false
		end)
	end
...
function CalcView(ply,vec,ang,fov,znear,zfar)
	...
	if wep.sightyes == false then
		if weaponClass == "weapon_glock18" then
			--Vector(3.85,10,1.45)
			vecWep = hand.Pos + hand.Ang:Up() * 1.6 - hand.Ang:Forward() * 10 + hand.Ang:Right() * 0.05
		...
		if weaponClass == "weapon_glock" then
			--Vector(2.3,10,0)
			vecWep = hand.Pos + hand.Ang:Up() * 1.7 - hand.Ang:Forward() * 10 + hand.Ang:Right() * 0.125
		...
		if weaponClass == "weapon_makarov" then
			--Vector(2.5,10,0.1)
			vecWep = hand.Pos + hand.Ang:Up() * 3.95 - hand.Ang:Forward() * 10 + hand.Ang:Right() * 1.45
		...
		if weaponClass == "weapon_de" then
			--Vector(3.9,10,1.09)
			vecWep = hand.Pos + hand.Ang:Up() * 1.9 - hand.Ang:Forward() * 10 + hand.Ang:Right() * 0.2
			angWep = hand.Ang + Angle(0,8,0)
		end
		if weaponClass == "weapon_akm" then
			--Vector(5.1,5,0.76)
			vecWep = hand.Pos + hand.Ang:Up() * 3.8 - hand.Ang:Forward() * 5 + hand.Ang:Right() * 0.95
		...
		if weaponClass == "weapon_l1a1" then
			--Vector(5.7,4,1.1)
			vecWep = hand.Pos + hand.Ang:Up() * 4.45 - hand.Ang:Forward() * 4 + hand.Ang:Right() * 0.8
			angWep = hand.Ang + Angle(0,0,0)
		end
		...
		if weaponClass == "weapon_minu14" then
			--Vector(5,4,0.95)
			vecWep = hand.Pos + hand.Ang:Up() * 6 - hand.Ang:Forward() * 4 + hand.Ang:Right() * 0.86
			angWep = hand.Ang + Angle(-5,7,150)
		end
