...
SWEP.Purpose = "Едет любимый зять,будем пить ПИВООООООООООООООООО."
SWEP.Category = "Вкусности"
SWEP.IconkaInv = "vgui/cola.png"
...
function SWEP:PrimaryAttack()
	...
	if(SERVER)then
		...
		self:GetOwner().Drunk = self:GetOwner().Drunk + 1
		self:GetOwner().pain = self:GetOwner().pain + 50
		...
		self:GetOwner().pain = self:GetOwner().pain + 400
