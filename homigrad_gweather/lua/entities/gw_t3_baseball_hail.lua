...
ENT.PrintName = "Baseball-Sized Hail"
...
function ENT:ScreenParticleEffects()
	...
	local ang= (dir==vector_origin and Angle(6-gWeather:GetWindSpeed(),dir:Angle()[2],0)) or dir:Angle()
	
	for k,ply in ipairs(player.GetAll()) do
	
		gWeather.Particles(ply,effect,Angle(6-math.min(gWeather:GetWindSpeed(),50),ang[2],0),0.8,600)
...
function ENT:Think()
	if (CLIENT) then
		...
		if LocalPlayer().gWeather.Sounds["IndoorHail"] then
			LocalPlayer().gWeather.Sounds["IndoorHail"]:ChangeVolume( (1-vol) * (4*vol) * 0.3,fdt) 
		end
